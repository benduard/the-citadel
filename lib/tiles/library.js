  ...
  function signedOutHTML() {
    return '<div class="acctTitle">Account</div>' +
      ...
      '<div class="acctCodeRow" hidden>' +
        '<input class="acctCodeInput" type="text" inputmode="numeric" pattern="[0-9]*" ' +
  ...
  function onAccountPanelClick(e) {
    ...
    if (act === 'send') {
      ...
      window.VitalityRemote.signInWithEmail(email).then(function (r) {
        ...
        if (msg) msg.textContent = 'Check your email. Type the 6-digit code below - it works everywhere, including a home-screen icon, where the link in that email cannot.'
