...
;(function () {
   ...
   *
   * `label` is the bare exercise name and stays that way: getActiveRestTimer()
   * hands it back to the tile, which feeds it into its own restFor() to redraw
   * a resumed countdown at the right length. `note` is the sentence the
   * notification reads out ("Barbell Bench Press, set 4. Last time 100 kg x
   * 8.") and is only ever printed. They are two columns because folding the
   * set number into the label would silently reprice every resumed timer.
   */
  /**
   * IS THIS FAILURE JUST "THAT COLUMN IS NOT THERE YET"?
   *
   * `note` was added to rest_timers on 2026-08-08, and a column only exists on
   * a project once supabase/push.sql has been RUN again. Between shipping the
   * code and running the SQL, PostgREST rejects the whole insert - so the timer
   * row is never written and the notification silently stops arriving, with
   * nothing on screen to say why. That is exactly what happened to Ruben.
   *
   * PostgREST says PGRST204 for an unknown column; Postgres itself says 42703.
   * Both are matched, and the message is checked too, because a schema-cache
   * error is worth exactly one narrow retry and nothing else.
   */
  function isMissingColumn(err, name) {
    ...
    return /schema cache|column/i.test(msg) && msg.indexOf(name) !== -1
  }

  function scheduleRestPush(seconds, label, note) {
    var c = getClient()
    if (!c) return Promise.resolve({ ok: false, reason: 'Sign in is not available on this board.' })
    ...
    return getSession().then(function (session) {
      ...
      var full = {}
      for (var k in base) if (Object.prototype.hasOwnProperty.call(base, k)) full[k] = base[k]
      full.note = note || null

      return c.from('rest_timers').insert(full).then(function (res) {
        if (!res || !res.error) return { ok: true }

        /**
         * THE ALERT MATTERS MORE THAN THE SENTENCE IN IT. A board whose SQL is
         * one migration behind should lose the set number, not the
         * notification. So the row goes in without `note` and the Edge
         * Function falls back to `label` - which is precisely the alert this
         * board sent before the set number was ever added.
         *
         * It is not silent: the console says which migration is missing, so
         * this degrades in the open rather than looking like it always worked.
         */
        if (isMissingColumn(res.error, 'note')) {
          warn('rest_timers has no `note` column yet - run supabase/push.sql again ' +
               'to get the set number in the alert. Scheduling without it for now.')
          return c.from('rest_timers').insert(base).then(function (r2) {
            if (r2 && r2.error) { warn('schedule rest push failed', r2.error); return { ok: false, reason: 'Could not schedule it.' } }
            return { ok: true, degraded: 'note' }
          })
        }

        warn('schedule rest push failed', res.error)
        return { ok: false, reason: 'Could not schedule it.' }
      })
  ...
  /* -------------------------------------------------------------------------
   * REMINDERS. supabase/reminders.sql, and the sender that reads it is
   * supabase/functions/send-reminder-push.
   *
   * WHY THESE ROWS EXIST WHEN EVERY OTHER TILE MANAGES WITH ONE JSON SLOT:
   * the thing that has to read a reminder is a cron job running at 07:00 while
   * the phone is face down. It cannot open the app to find out what he wanted,
   * so the rule has to be in columns something can query.
   *
   * THE TILE IS THE SOURCE OF TRUTH AND THIS IS A ONE-WAY PROJECTION OF IT.
   * The tile keeps its list in its own vault slot like every other tile, and
   * every change pushes the WHOLE list down here: upsert what is in it, delete
   * what is not. No merge, no conflict resolution, no second copy that can
   * disagree - the same last-write-wins the slots have always had.
   *
   * THE SERVER'S OWN TWO COLUMNS ARE NEVER SENT. last_fired_key is written by
   * the sender and is the only thing stopping a reminder going twice; it is
   * absent from the payload, so PostgREST leaves it alone on conflict. Send it
   * and every sync would rearm every reminder that already went off today.
   * ------------------------------------------------------------------------- */

  /**
   * IS THIS FAILURE JUST "THAT TABLE IS NOT THERE YET"?
   *
   * Same window, same lesson as the `note` column (see isMissingColumn and
   * vault/decisions.md): the code deploys on push, the SQL is run by hand in a
   * browser, and there is always a gap between the two. The difference here is
   * that a missing table is not something to degrade around - there is nowhere
   * to write - so it is named instead, and the tile says which file to run
  ...
  function isMissingTable(err) {
    ...
    var msg = String(err.message || '') + ' ' + String(err.details || '')
    return /schema cache|does not exist|relation/i.test(msg) && /reminders/.test(msg)
  }

  var NO_TABLE = 'Your vault has no reminders table yet. Run supabase/reminders.sql ' +
                 'in the Supabase SQL editor, and these will start arriving.'

  /**
   * Push the whole list. `list` is the tile's own reminders, already in the
   * shape the table wants.
   *
   * Resolves { ok } or { ok:false, reason, needsMigration? }. It NEVER resolves
   * ok on a write it did not do: a reminders tile that says "saved" while
   * nothing reaches the server is the exact failure that took the rest alert
   * down for a day and read as "notifications are broken".
   */
  function syncReminders(list) {
    var c = getClient()
    if (!c) return Promise.resolve({ ok: false, reason: 'Sign in is not available on this board.' })
    if (!Array.isArray(list)) return Promise.resolve({ ok: false, reason: 'Nothing to sync.' })

    return getSession().then(function (session) {
      if (!session || !session.user) {
        return { ok: false, signedOut: true,
          reason: 'Signed out. A reminder has to live in your vault to reach your phone.' }
      }
      var uid = session.user.id
      ...
      var rows = list.map(function (r) {
        return {
          ...
          at_time: String(r.time || ''),
          tz: String(r.tz || 'UTC'),
          repeat: String(r.repeat || 'once'),
          days: Array.isArray(r.days) ? r.days : [],
          on_date: r.repeat === 'once' ? (r.date || null) : null,
          enabled: !!r.enabled,
          sticky: !!r.sticky,
          // The tile's own stamp, bumped when THAT reminder is edited and
          // never when its neighbour is. The sender refuses to fire an
          // occurrence older than this, which is what stops a reminder set at
          // 08:00 for 07:30 going off the instant it is saved.
          updated_at: r.updatedAt || new Date().toISOString()
        }
      })

      // DELETE FIRST, and only the ones actually gone. `not in` with an empty
      // list is a syntax error in PostgREST, so an emptied list is its own
      // path - and it genuinely means "delete them all", which is what
      // removing your last reminder is.
      var ids = rows.map(function (r) { return r.id })
      var del = ids.length
        ? c.from('reminders').delete().eq('user_id', uid).not('id', 'in', '(' + ids.map(function (i) {
            // PostgREST splits this list on commas, so an id containing one
            // would silently shorten it and delete a reminder he still has.
            // The tile only ever mints [a-z0-9], and this makes that a rule
            // rather than a hope.
            return String(i).replace(/[(),"]/g, '')
          }).join(',') + ')')
        : c.from('reminders').delete().eq('user_id', uid)

      return del.then(function (dres) {
        if (dres && dres.error) return { ok: false, error: dres.error }
        if (!rows.length) return { ok: true }
        return c.from('reminders').upsert(rows, { onConflict: 'user_id,id' }).then(function (ures) {
          if (ures && ures.error) return { ok: false, error: ures.error }
          return { ok: true }
        })
      }).then(function (res) {
        if (res.ok) return { ok: true, count: rows.length }
        if (isMissingTable(res.error)) {
          warn('the reminders table is missing - run supabase/reminders.sql', res.error)
          return { ok: false, needsMigration: true, reason: NO_TABLE }
        }
        warn('sync reminders failed', res.error)
        return { ok: false, reason: 'Could not save these to your vault, so they will not reach your phone.' }
      })
    }).catch(function (e) {
      warn('sync reminders threw', e)
      return { ok: false, reason: 'Could not reach the vault. Nothing was saved there.' }
    })
  }

  /**
   * Read back only what the SERVER knows and the tile cannot: whether each
   * reminder has actually been sent, and when.
   *
   * This is not a second copy of the list and must never be treated as one -
   * the tile already holds the list. It exists so the tile can say "sent this
   * morning at 07:00" out of the record rather than out of hope. The whole
   * reason the rest alert could be dead for a day without anyone noticing was
   * that every visible thing looked healthy; a reminder that never left the
   * building should say so on its own face.
   *
   * Resolves { ok:true, fired:{ id: key } } or { ok:false }.
   */
  function readReminders() {
    var c = getClient()
    if (!c) return Promise.resolve({ ok: false })
    return getSession().then(function (session) {
      if (!session || !session.user) return { ok: true, fired: {} }
      return c.from('reminders')
        .select('id, last_fired_key, enabled')
        .eq('user_id', session.user.id)
        .then(function (res) {
          if (res && res.error) {
            if (isMissingTable(res.error)) return { ok: false, needsMigration: true }
            warn('read reminders failed', res.error)
            return { ok: false }
          }
          var fired = {}
          ;(res.data || []).forEach(function (row) {
            if (row.last_fired_key) fired[row.id] = row.last_fired_key
          })
          return { ok: true, fired: fired }
        })
    }).catch(function (e) { warn('read reminders threw', e); return { ok: false } })
  }
