...
// ── 14. down-is-better streams are not scored upside down ──────────────────
//
// Added 2026-09-07 with the Screen time tile, which is the first stream on
// this board where FEWER is the win. trendScore() reads any rise as an
// improvement, so without spec.dir a worsening phone habit would have pushed
// the rank UP the more he scrolled - a bug that would have looked like the
// board working, and would have quietly rewarded the exact thing the tile was
// added to watch.
console.log('\n[14] a falling number can be the good direction')
const stDays = (recent, prior) => {
  const rows = []
  for (let d = 0; d < 28; d++) {
    rows.push({ key: 'screen_minutes', value: d < 14 ? recent : prior, date: iso(d), tile: 'screentime' })
  }
  return rows
}

r = Rank.compute(stDays(180, 300), NOW)
let ts = r.parts.find(p => p.id === 'screentime')
check('screen time is in the rank at all', !!ts, JSON.stringify(r.parts.map(p => p.id)))
check('fewer minutes than a fortnight ago reads as IMPROVING', ts.trend > 0.5, String(ts.trend))
check('and it carries its direction out for the panel', ts.dir === 'down', ts.dir)
check('the raw numbers are untouched by the flip',
  ts.detail.recent === 180 && ts.detail.prior === 300, JSON.stringify(ts.detail))

r = Rank.compute(stDays(300, 180), NOW)
ts = r.parts.find(p => p.id === 'screentime')
check('more minutes than a fortnight ago reads as SLIDING', ts.trend < 0.5, String(ts.trend))

// The mirror has to be exact, or the same size of change would be worth more
// in one direction than the other and the rank would drift over time.
const up = Rank.compute(stDays(180, 300), NOW).parts.find(p => p.id === 'screentime').trend
const down = Rank.compute(stDays(300, 180), NOW).parts.find(p => p.id === 'screentime').trend
check('the flip is an exact mirror about the middle',
  Math.abs((up + down) - 1) < 1e-9, `${up} + ${down}`)

// And the streams that were always up-is-better must be untouched by any of it.
r = Rank.compute(enough, NOW)
tc = r.parts.find(p => p.id === 'checkin')
check('an up-is-better stream still reads a rise as improving', tc.trend > 0.5, String(tc.trend))
check('and defaults its direction to up', tc.dir === 'up', tc.dir)
