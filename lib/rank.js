 ...
 *
 *   checkin     (20 + 32 + 36) / 3 = 29.3  ->  29
 *   lifting     (40 + 18 + 22) / 3 = 26.7  ->  27
 *   recovery    (30 + 27 + 14) / 3 = 23.7  ->  24
 *   projects    ( 0 +  9 + 18) / 3 =  9.0  ->   9
 *   screentime  ( 0 + 10 + 10) / 3 =  6.7  ->   7
 *   body        (10 +  4 +  0) / 3 =  4.7  ->   4
 *                                            ------
 *                                              100
 *
 * Recomputed 2026-09-07, when screen time was added to weights.ts.
 *
 * THE ROUNDING IS NOT free-hand. The exact averages sum to 100 and the
 * rounded ones did not - four of them sat on .67 and rounding all four up
 * gave 101. Largest remainder decides it: floor everything, then hand the
 * three spare points to the three biggest remainders, ties going to the
 * larger weight. That is why body is 4 rather than 5. Do the same if these
 * are ever recomputed, or the "weights sum to 100" test in lib/rank.test.js
 * will catch you and it will not tell you which one to bend.
 *
...
;(function (root, factory) {
  ...
  var CADENCE_PER_WEEK = {
    ...
    lifting: 4,
    body: 3,
    // 7, because the number is a whole day and every day has one. It is also
    // the one cadence here that is mostly not his to hit: the Mac export runs
    // on a schedule, so a gap in this row usually means the export stopped,
    // not that he stopped. Worth knowing before reading a dip in it as a
    // habit changing.
    screentime: 7,
   ...
   *
   * `dir` is which way is BETTER, and it defaults to 'up' because every
   * stream on this board did until screen time arrived. It is not cosmetic:
   * trendScore() reads a rise as an improvement, so a down-is-better number
   * with no dir would score a worsening phone habit as progress and quietly
   * push the rank UP the more he scrolled. Anything added here that is better
   * when it falls must say so.
   */
  var TILES = [
    ...
    // The tile is called "Lists" on the board now, but this row is labelled
    // Projects because that is what the number counts: projects_done, not
    // groceries ticked off. Naming it Lists would overstate what it measures.
    ...
    // The first stream on this board where DOWN is the good direction, which
    // is the whole reason `dir` exists. Unlike body_weight, the intent here
    // was actually stated when the tile was added - it is weighted into "feel
    // good" and "show up" in weights.ts, and both improve as the hours fall -
    // so this is recorded intent, not this file inventing a goal.
    { id: 'screentime', key: 'screen_minutes', label: 'Screen time', mode: 'daily-mean', unit: 'm', dir: 'down' }
  ]
  ...
  function scoreTile(spec, rows, today) {
    ...
    /**
     * DOWN IS BETTER, flipped here rather than inside trendScore().
     *
     * trendScore maps the change linearly onto 0..1 and is symmetric about
     * 0.5 - flat is exactly the middle, a full improvement is 1, a full slide
     * is 0 - so `1 - t` is the exact mirror and not an approximation of one.
     * It holds at the edges too: the prior === 0 case returns 1 for "from
     * nothing to something", which for a falling metric is the worst possible
     * fortnight, and 1 - 1 is 0. A null stays null, because not knowing does
     * not become knowing when you turn it around.
     *
     * Kept out of trendScore because that function is also fed raw pairs by
     * the tests, and a direction-aware helper is much easier to call wrong.
     */
    if (spec.dir === 'down' && trend != null) trend = 1 - trend
    ...
    return {
      id: spec.id, label: spec.label, key: spec.key, unit: spec.unit, mode: spec.mode,
      // Carried out so the rank panel can point the arrow at what the NUMBERS
      // did while colouring it by whether that was good. For screen time
      // those two disagree, which is the entire reason this is exported.
      dir: spec.dir || 'up',
