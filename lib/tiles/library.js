  ...
  function onAccountPanelClick(e) {
    ...
    if (act === 'send') {
      ...
      window.VitalityRemote.migrateLocalData().then(function (r) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
        // Never celebrate a write that did not land, and never mark the ask
        // answered when it failed - the data is still only on this device, so
        // the offer must come back.
        if (!r || !r.ok) {
          if (msg) msg.textContent = 'Some of it did not upload. Your data is still safe on this device.'
          return
        }
        markMigrated(who)
        // A skipped tile is not a failure, it is the account already having
        // that tile. Say which, rather than quietly doing nothing to it.
        if (r.skipped.length) {
          if (msg) {
            msg.textContent = 'Kept what your account already had for: ' + r.skipped.join(', ') +
              '. Nothing was overwritten.'
          }
          toast(r.uploaded.length ? 'Uploaded what was missing' : 'Your account already had this')
          return
        }
        refreshAccount()
        toast('Your local data is in your account now')
      }).catch(function (e) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
