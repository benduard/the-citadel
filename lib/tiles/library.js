...
;(function () {
  ...
  var IC = {
    ...
    account: svg('<circle cx="12" cy="8.2" r="3.4"/><path d="M5 19.5c0-3.6 3.1-6 7-6s7 2.4 7 6"/>', 1.7),
    // tile glyphs
  ...
  function injectStyles() {
    ...
    s.id = 'vlibStyles'
    s.textContent = [
      ':root{--sheet:#0b0d10;--elevated:#151a20;--card-2:rgba(255,255,255,.04);--gold:#c8a567;--petrol:#2e7d78;--petrol-hover:#266864;--petrol-ink:#04211e;--spring:cubic-bezier(.34,1.56,.64,1)}',
      /* gear */
      ...
      '.vlibGear:hover{color:var(--petrol,#2e7d78);border-color:rgba(46,125,120,.5)}.vlibGear:active{transform:scale(.94)}.vlibGear svg{width:20px;height:20px;display:block}',
      ...
      ".title{font-weight:700;font-size:1.05rem;letter-spacing:.14em;text-transform:uppercase;line-height:1;display:flex;align-items:center;gap:12px}",
      '.title .lock{display:inline-flex;width:22px;height:22px;color:var(--petrol);margin-top:3px}',
      ...
      '.nav:hover{background:var(--card-2);color:var(--fg)}.nav:hover .gi{color:var(--muted-strong)}.nav.on{background:rgba(46,125,120,.1);color:var(--fg)}.nav.on .gi{color:var(--petrol)}.nav.on .cnt{color:var(--muted-strong)}',
      '.nav.on::before{content:"";position:absolute;left:-12px;top:50%;transform:translateY(-50%);width:3px;height:18px;border-radius:0 3px 3px 0;background:var(--petrol)}',
      '.swatch{width:9px;height:9px;border-radius:3px;flex:none}.swatch.core{background:var(--gold)}.swatch.mine{background:var(--petrol)}',
      ...
      '.item + .item{border-top:1px solid var(--border)}.item:hover{background:var(--card-2);border-color:rgba(46,125,120,.28);box-shadow:0 0 44px rgba(46,125,120,.07);transform:translateY(-2px)}.item:hover + .item{border-top-color:transparent}',
      ...
      '.tileicon.core{color:var(--gold);border-color:rgba(200,165,103,.32);background:linear-gradient(160deg,rgba(200,165,103,.14),rgba(200,165,103,.02))}.tileicon.mine{color:var(--petrol);border-color:rgba(46,125,120,.3);background:linear-gradient(160deg,rgba(46,125,120,.12),rgba(46,125,120,.02))}',
      '.item[data-state="dash"] .tileicon::after{content:"";position:absolute;top:-3px;right:-3px;width:8px;height:8px;border-radius:50%;background:var(--petrol);border:2px solid var(--sheet);box-shadow:0 0 7px rgba(46,125,120,.8);animation:livingDot 2.6s ease-in-out infinite}@keyframes livingDot{0%,100%{opacity:.55}50%{opacity:1}}',
      ...
      '.a{font-size:.78rem;font-weight:600;letter-spacing:-.01em;border-radius:9px;padding:7px 11px;cursor:pointer;border:1px solid var(--border-strong);background:var(--card-2);color:var(--fg);white-space:nowrap;transition:background .16s,border-color .16s,transform .12s}.a:hover{border-color:var(--petrol)}.a:active{transform:scale(.95)}',
      '.a.add{background:var(--petrol);color:var(--petrol-ink);border-color:transparent}.a.add:hover{background:var(--petrol-hover)}.a.sub{color:var(--muted-strong);background:transparent}.a.del{color:var(--amber);border-color:rgba(245,158,11,.36);background:rgba(245,158,11,.05)}.a.del:hover{border-color:var(--amber)}',
      '.a.del.armed{background:var(--amber);color:#1a1206;border-color:transparent;animation:armPulse 1.1s ease-in-out infinite}@keyframes armPulse{0%,100%{opacity:1}50%{opacity:.72}}',
      ...
      '.main[data-view="gallery"] .item[data-cat="core"] .art{background:linear-gradient(150deg,rgba(200,165,103,.16),rgba(200,165,103,.02))}.main[data-view="gallery"] .item[data-cat="intake"] .art,.main[data-view="gallery"] .item[data-cat="count"] .art,.main[data-view="gallery"] .item[data-cat="duration"] .art,.main[data-view="gallery"] .item[data-cat="measure"] .art{background:linear-gradient(150deg,rgba(46,125,120,.16),rgba(46,125,120,.02))}',
      ...
      '.vtoast{position:fixed;left:50%;bottom:30px;transform:translateX(-50%) translateY(20px);z-index:95;display:none;align-items:center;gap:10px;padding:12px 18px;border-radius:14px;border:1px solid rgba(46,125,120,.28);background:rgba(10,10,10,.92);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);box-shadow:0 18px 48px -18px rgba(0,0,0,.9),0 0 44px rgba(46,125,120,.07);color:var(--fg);font-size:.8125rem;font-weight:600;white-space:nowrap}.vtoast.show{display:flex;animation:toastIn .42s var(--spring) both}@keyframes toastIn{from{opacity:0;transform:translateX(-50%) translateY(20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}.vtoast .td{width:18px;height:18px;border-radius:50%;display:grid;place-items:center;background:rgba(46,125,120,.16);color:var(--petrol);flex:none}.vtoast .td svg{width:11px;height:11px}',
      /* account */
      '.acctWrap{position:relative}',
      '.acctPanel{position:absolute;top:calc(100% + 10px);right:0;z-index:70;width:280px;padding:18px;border-radius:16px;border:1px solid var(--border-strong);background:var(--elevated);box-shadow:0 24px 60px -20px rgba(0,0,0,.85);animation:fadeUp .28s var(--ease-premium) both}',
      '.acctTitle{font-size:.72rem;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:12px;font-weight:600}',
      '.acctEmailInput{width:100%;background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 12px;color:var(--fg);font-size:.8125rem;margin-bottom:10px}',
      '.acctEmailInput::placeholder{color:var(--muted)}',
      '.acctCodeRow{margin-top:14px;padding-top:14px;border-top:1px solid var(--border)}',
      '.acctCodeInput{width:100%;background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px 12px;color:var(--fg);font-size:1rem;letter-spacing:.28em;text-align:center;font-variant-numeric:tabular-nums;margin-bottom:10px}',
      '.acctCodeInput::placeholder{color:var(--muted);letter-spacing:normal}',
      '.acctSignedAs{font-size:.8125rem;color:var(--fg);margin-bottom:12px;word-break:break-all}',
      '.acctMsg{font-size:.75rem;color:var(--muted);margin-top:10px;line-height:1.4}',
      '.acctNote{font-size:.8125rem;color:var(--muted-strong);line-height:1.45;margin-bottom:14px}',
      // Amber, never red: this is a "here is what happened," not a scolding.
      '.acctNotice{margin-bottom:14px;padding:10px 12px;border-radius:10px;border:1px solid rgba(245,158,11,.36);background:rgba(245,158,11,.07);color:var(--fg);font-size:.78125rem;line-height:1.45}',
      '.acctPanel .a{width:100%;text-align:center;margin-bottom:8px}.acctPanel .a:last-child{margin-bottom:0}',
      '.mside{display:none}',
      '@media(max-width:860px){.win{height:94vh;grid-template-rows:auto auto 1fr}.chrome{grid-template-columns:1fr;--rail:0}.brand{border-right:0;padding-bottom:16px}.toolbar{border-top:1px solid var(--border);padding:14px 18px;flex-wrap:wrap;gap:10px}.body{grid-template-columns:1fr}.side{display:none}.mside{display:flex;gap:8px;overflow-x:auto;padding:14px 18px 4px;border-bottom:1px solid var(--border)}.mside::-webkit-scrollbar{display:none}.mside .pill{flex:none;min-height:40px;display:inline-flex;align-items:center;gap:7px;border:1px solid var(--border);background:var(--card);color:var(--muted-strong);font-size:.8125rem;font-weight:600;padding:8px 14px;border-radius:999px;cursor:pointer;white-space:nowrap}.mside .pill.on{background:rgba(46,125,120,.12);color:var(--fg);border-color:rgba(46,125,120,.4)}.mside .pill .cnt{color:var(--muted)}.cols{grid-template-columns:1fr 110px 100px 200px}.acts{opacity:1}.a{min-height:40px;display:inline-flex;align-items:center}}',
      '@media(max-width:560px){.scrim{padding:10px 8px}.win{border-radius:20px;height:96vh}.title{font-size:1.85rem}.cols{display:none}.item{grid-template-columns:1fr auto;grid-template-areas:"name chip" "meta meta";gap:10px 12px;padding:15px 14px}.item:hover{transform:none}.cell-name{grid-area:name}.chip-cell{grid-area:chip;justify-self:end;align-self:center}.item .meta-row{grid-area:meta;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;padding-left:51px}.acts{opacity:1!important;justify-content:flex-start;flex-wrap:wrap}.a{min-height:40px;background:transparent}.a.add{background:var(--petrol)}}',
  ...
  function render() {
    if (!overlay) return
    disarmDelete() // the row about to be rebuilt cannot stay armed underneath it
  ...
  // ── Account: sign in, sign out, and the one-time "bring your local data
  //    with you" ask (never silently overwritten - the house rule). ─────────
  // Keyed BY ACCOUNT, not globally: answering "start fresh" for one account
  // must not silently skip the ask for a different one signed in later.
  function migrateKey(userId) { return 'v:migrated:' + userId }
  function alreadyMigrated(userId) {
    if (!userId) return false
    try { return !!window.localStorage.getItem(migrateKey(userId)) } catch (e) { return false }
  }
  function markMigrated(userId) {
    // No account, nothing to file the answer against. Writing 'v:migrated:null'
    // would answer the ask for a user that does not exist while leaving the
    // real one still unanswered.
    if (!userId) return
    try {
      window.localStorage.setItem(migrateKey(userId), '1')
      // The local snapshot this offer was about now belongs to the answer
      // this account gave it - claimed, not free for the next account that
      // signs in on this device to be offered as if it were theirs.
      window.localStorage.setItem(LOCAL_CLAIMED_KEY, '1')
    } catch (e) {}
  }
  // A save while signed in writes straight to the account, never back to this
  // key, so what is here is a SNAPSHOT frozen at the moment someone last
  // decided what to do with it - not live data. Once any account has decided
  // (upload or start fresh), it is claimed: offering it again to a DIFFERENT
  // account would hand that account a stranger's old list under "your local
  // data," not a promise this board makes.
  var LOCAL_CLAIMED_KEY = 'v:local-claimed'
  function localClaimed() {
    try { return !!window.localStorage.getItem(LOCAL_CLAIMED_KEY) } catch (e) { return false }
  }
  function shouldOfferMigration(userId) {
    return !alreadyMigrated(userId) && !localClaimed() && window.VitalityRemote && window.VitalityRemote.hasLocalData()
  }

  function signOutNoticeHTML() {
    if (!hadUnexpectedSignOut()) return ''
    return '<div class="acctNotice">' +
      'You got signed out on this device without asking to be - your board is running on local ' +
      'storage until you sign in again.' +
      '<button class="a sub" data-act="dismiss-signout-notice" style="width:100%;margin-top:8px">Got it</button>' +
      '</div>'
  }

  function signedOutHTML() {
    return '<div class="acctTitle">Account</div>' +
      signOutNoticeHTML() +
      '<input class="acctEmailInput" type="email" placeholder="you@email.com" autocomplete="email">' +
      '<button class="a add" data-act="send" style="width:100%">Send code</button>' +
      '<div class="acctMsg"></div>' +
      // Hidden until a code has actually been requested - typing a code with
      // nothing sent yet has nothing valid to check against. On a phone with
      // this board added to the home screen, the email's link opens regular
      // Safari (a different window from the installed icon) and can never
      // sign the icon in; typing the code here always works because nothing
      // has to hand off to another window.
      '<div class="acctCodeRow" hidden>' +
        // No fixed length: Supabase's own OTP length is a project setting,
        // not something this file can know in advance, and guessing it
        // wrong silently truncates whatever the person actually typed.
        '<input class="acctCodeInput" type="text" inputmode="numeric" ' +
          'placeholder="code from email" autocomplete="one-time-code">' +
        '<button class="a add" data-act="verify" style="width:100%">Confirm code</button>' +
      '</div>'
  }
  function signedInHTML(email) {
    return '<div class="acctTitle">Account</div>' +
      '<div class="acctSignedAs">Signed in as ' + esc(email) + '</div>' +
      // The emergency protocol. If the Supabase project is ever deleted or
      // lost, the file this writes IS the board - every tile and every
      // ledger row, on your own machine, readable without this app.
      '<button class="a" data-act="backup" style="width:100%">Back up everything to this device</button>' +
      // Rest-timer push lives HERE, in the host, and not in the Lifting tile
      // that owns the timer - because it cannot live there. A sealed tile has
      // an opaque origin: touching localStorage throws, reaching the parent
      // throws, and registering a service worker is no different. Only the
      // host can subscribe, so the switch is where the subscribing happens.
      '<button class="a" data-act="push" style="width:100%">Rest timer alerts</button>' +
      '<button class="a sub" data-act="signout" style="width:100%">Sign out</button>' +
      '<div class="acctMsg"></div>'
  }
  function migrateHTML(email) {
    return '<div class="acctTitle">Account</div>' +
      '<div class="acctSignedAs">Signed in as ' + esc(email) + '</div>' +
      '<div class="acctNote">You have data saved on this device. Bring it into this account, or start fresh here?</div>' +
      '<button class="a add" data-act="upload" style="width:100%">Upload my local data</button>' +
      '<button class="a sub" data-act="skip" style="width:100%">Start fresh</button>' +
      '<div class="acctMsg"></div>'
  }

  /**
   * Write the export to the user's own disk.
   *
   * Deliberately a plain Blob + <a download> rather than anything server
   * side: the file never leaves the browser, so a backup of the whole vault
   * is not also a copy of the whole vault sitting on somebody else's
   * machine. Dated filename so successive backups sit beside each other
   * instead of overwriting - the one that is three months old is exactly the
   * one you want when today's data turns out to be wrong.
   */
  function backupFilename(iso) {
    return 'citadel-backup-' + String(iso || '').slice(0, 10) + '.json'
  }
  /**
   * The push button says what it will DO, not what it is. "Rest timer alerts"
   * on a switch gives no clue which way it is about to go; "Turn on" / "Turn
   * off on this device" does. Read from the browser every time the panel
   * opens, because permission can be revoked in system settings without this
   * board ever hearing about it.
   */
  function refreshPushLabel(panel) {
    if (!panel || !window.VitalityPush) return
    var btn = panel.querySelector('[data-act="push"]')
    if (!btn) return
    var s = window.VitalityPush.support()
    if (!s.ok) {
      btn.textContent = 'Rest timer alerts unavailable'
      btn.disabled = true
      btn.title = s.why
      return
    }
    window.VitalityPush.isEnabled().then(function (on) {
      btn.textContent = on ? 'Turn off rest timer alerts' : 'Turn on rest timer alerts'
    }).catch(function () {})
  }
  ...
  function downloadBackup(payload) {
    ...
    a.click()
    a.remove()
    // Freeing it immediately can cancel the download mid-write in some
    // browsers; a beat later is safe and the object is small.
    setTimeout(function () { URL.revokeObjectURL(url) }, 4000)
  }

  // Who is signed in right now, so the migration ask and its answer are filed
  // against the right account.
  var currentUserId = null

  function refreshAccount() {
    if (!overlay) return
    var panel = overlay.querySelector('#vlibAcctPanel')
    if (!panel) return
    if (!window.VitalityRemote) {
      panel.innerHTML = '<div class="acctTitle">Account</div><div class="acctMsg">Sync is not available on this board yet.</div>'
      return
    }
    // Offering a sign in form that cannot possibly work would be a lie. Say
    // what is actually wrong instead.
    if (!window.VitalityRemote.isAvailable()) {
      panel.innerHTML = '<div class="acctTitle">Account</div>' +
        '<div class="acctMsg">Could not reach the sign in service. Your board is running on this device only. ' +
        'Check your connection, then reload.</div>'
      return
    }
    window.VitalityRemote.getSession().then(function (session) {
      if (session && session.user) {
        currentUserId = session.user.id
        panel.innerHTML = shouldOfferMigration(currentUserId)
          ? migrateHTML(session.user.email)
          : signedInHTML(session.user.email)
        refreshPushLabel(panel)
      } else {
        currentUserId = null
  ...
  function toggleAccountPanel() {
    ...
    if (willShow) refreshAccount()
  }

  function onAccountPanelClick(e) {
    var panel = overlay.querySelector('#vlibAcctPanel')
    var b = e.target.closest ? e.target.closest('[data-act]') : null
    if (!b || !panel || !panel.contains(b)) return
    var act = b.getAttribute('data-act')

    if (act === 'send') {
      var input = panel.querySelector('.acctEmailInput')
      var email = input ? input.value.trim() : ''
      if (!email) return
      b.disabled = true
      window.VitalityRemote.signInWithEmail(email).then(function (r) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
        if (r && r.error) { if (msg) msg.textContent = r.error.message || 'Something went wrong. Try again.'; return }
        if (msg) msg.textContent = 'Check your email. Type the code below - it works everywhere, including a home-screen icon, where the link in that email cannot.'
        // The code row has no email field of its own; it verifies against
        // whichever address the code was actually sent to.
        panel.dataset.pendingEmail = email
        var row = panel.querySelector('.acctCodeRow')
        if (row) row.hidden = false
        var codeInput = panel.querySelector('.acctCodeInput')
        if (codeInput) codeInput.focus()
      }).catch(function (e) {
        // A rejected promise must never strand the button disabled.
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
        if (msg) msg.textContent = (e && e.message) ? e.message : 'Could not reach the vault. Try again.'
      })
    } else if (act === 'verify') {
      var codeIn = panel.querySelector('.acctCodeInput')
      var code = codeIn ? codeIn.value.trim() : ''
      var pendingEmail = panel.dataset.pendingEmail || ''
      if (!code || !pendingEmail) return
      b.disabled = true
      window.VitalityRemote.verifyCode(pendingEmail, code).then(function (r) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
        if (r && r.error) { if (msg) msg.textContent = r.error.message || 'That code did not work. Try again.'; return }
        // A successful verify already fires SIGNED_IN through onAuthChange,
        // which is what actually flips the account panel over - this call is
        // just to stop the checked/disabled state lingering on screen.
        refreshAccount()
      }).catch(function (e) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
      ...
      var msgB = panel.querySelector('.acctMsg')
      b.disabled = true
      if (msgB) msgB.textContent = 'Reading everything in your vault...'
      window.VitalityRemote.exportEverything().then(function (r) {
        b.disabled = false
        if (!r || !r.ok) {
          if (msgB) msgB.textContent = (r && r.reason) || 'Could not build the backup.'
          return
        }
        downloadBackup(r)
        if (msgB) {
          msgB.textContent = 'Saved ' + r.counts.slots + ' ' + (r.counts.slots === 1 ? 'tile' : 'tiles') +
            ' and ' + r.counts.ledger + ' ledger ' + (r.counts.ledger === 1 ? 'row' : 'rows') +
            ' to your downloads. Keep it somewhere that is not this laptop too.'
        }
      }).catch(function () {
        b.disabled = false
        if (msgB) msgB.textContent = 'Could not reach the vault. Nothing was written.'
      })
    } else if (act === 'push') {
      var msgP = panel.querySelector('.acctMsg')
      if (!window.VitalityPush) {
        if (msgP) msgP.textContent = 'Notifications are not wired into this board.'
        return
      }
      b.disabled = true
      // Read the browser rather than a saved flag, so permission revoked in
      // system settings is reflected instead of a stale "on".
      window.VitalityPush.isEnabled().then(function (on) {
        if (on) {
          if (msgP) msgP.textContent = 'Turning them off on this device...'
          return window.VitalityPush.disable().then(function (r) {
            if (msgP) msgP.textContent = (r && r.ok)
              ? 'Off on this device. Any other device you turned them on stays on.'
              : ((r && r.reason) || 'Could not turn them off.')
          })
        }
        if (msgP) msgP.textContent = 'Asking for permission...'
        return window.VitalityPush.enable().then(function (r) {
          if (msgP) msgP.textContent = (r && r.ok)
            ? 'On. A rest timer will now reach you with the phone locked, give or take about fifteen seconds.'
            : ((r && r.reason) || 'Could not turn them on.')
        })
      }).then(function () {
        b.disabled = false
        refreshPushLabel(panel)
        ...
        b.disabled = false
        if (msgP) msgP.textContent = 'Something went wrong. Nothing was changed.'
      })
    } else if (act === 'signout') {
      window.VitalityRemote.signOut().then(function () { refreshAccount() })
    } else if (act === 'dismiss-signout-notice') {
      clearUnexpectedSignOut()
      refreshAccount()
    } else if (act === 'upload') {
      var who = currentUserId
      b.disabled = true
      window.VitalityRemote.migrateLocalData().then(function (r) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')

        // What actually decides the offer is whether the TILES landed. A
        // ledger row can fail for reasons that will never improve - a legacy
        // row with a broken date or a value that is not a number is rejected
        // by the same guards every time - and hanging the whole migration on
        // it left the ask coming back forever with no way to ever answer it.
        if (!r || !r.slotsOk) {
          if (msg) msg.textContent = 'Some of it did not upload. Your data is still safe on this device.'
          return
        }
        markMigrated(who)
        ...
        var note = ''
        if (r.skipped.length) {
          note = 'Kept what your account already had for: ' + r.skipped.join(', ') + '. Nothing was overwritten.'
        }
        if (r.ledgerFailed) {
          note += (note ? ' ' : '') + r.ledgerFailed + ' old log ' +
            (r.ledgerFailed === 1 ? 'row was' : 'rows were') +
            ' skipped as unreadable. Everything else is in.'
        }
        if (msg && note) msg.textContent = note

        // toast() reads through `overlay`, and closing the Library sets that
        // to null. Uploading then closing before it finished used to throw
        // here, land in the catch below, and report a migration that had
        // ALREADY succeeded as a failure.
        if (overlay) {
          if (r.skipped.length) toast(r.uploaded.length ? 'Uploaded what was missing' : 'Your account already had this')
          else toast('Your local data is in your account now')
        }
        if (!r.skipped.length) refreshAccount()
      }).catch(function (e) {
        b.disabled = false
        var msg = panel.querySelector('.acctMsg')
        if (msg) msg.textContent = 'Could not reach the vault. Your data is still safe on this device.'
      })
    } else if (act === 'skip') {
      markMigrated(currentUserId)
      refreshAccount()
    }
  }

  // ── Arm-then-confirm delete. No modal, no native confirm() (off design
  //    law); the button itself becomes the question for a few seconds.
  var armedDeleteBtn = null
  var armedDeleteTimer = null
  function armDelete(b) {
    armedDeleteBtn = b
    ...
    armedDeleteTimer = setTimeout(disarmDelete, 3000)
  }
  function disarmDelete() {
    if (armedDeleteBtn) {
      armedDeleteBtn.removeAttribute('data-armed')
      armedDeleteBtn.classList.remove('armed')
      if (armedDeleteBtn.dataset.label) armedDeleteBtn.textContent = armedDeleteBtn.dataset.label
    }
    clearTimeout(armedDeleteTimer)
    armedDeleteBtn = null
  }
  ...
  var openedFrom = null // the element to hand focus back to on close
  ...
  function open() {
      ...
      '<div class="win">' +
        '<div class="chrome">' +
          ...
          '<div class="toolbar">' +
            ...
            '<div class="acctWrap">' +
              '<button class="x" id="vlibAcctBtn" aria-label="Account" title="Account">' + IC.account + '</button>' +
              '<div class="acctPanel" id="vlibAcctPanel" hidden></div>' +
            '</div>' +
    ...
    // events
    overlay.querySelector('[aria-label="Close"]').addEventListener('click', close)
    ...
    overlay.querySelector('.body').addEventListener('click', function (e) {
      ...
      // Delete has no undo (act('delete') files the id away as gone for
      // good), so one tap arms it and a second tap on that SAME button
      // confirms it. Any other click in the dialog - a different row,
      // empty space, another action - disarms it silently instead of
      // deleting something the user only meant to look at.
      if (armedDeleteBtn && armedDeleteBtn !== b) disarmDelete()

      if (!b) return
      var kind = b.getAttribute('data-act')
      if (kind === 'delete' && !b.hasAttribute('data-armed')) {
        armDelete(b)
        return
      }
      if (kind === 'delete') disarmDelete() // this click IS the confirm
      act(kind, b.getAttribute('data-id'))
    })
    overlay.querySelector('#vlibAcctBtn').addEventListener('click', toggleAccountPanel)
    overlay.querySelector('#vlibAcctPanel').addEventListener('click', onAccountPanelClick)
    overlay.querySelector('#vlibAcctPanel').addEventListener('keydown', function (e) {
      if (e.key !== 'Enter') return
      var target = e.target
      if (target.classList.contains('acctEmailInput')) {
        var sendBtn = overlay.querySelector('#vlibAcctPanel [data-act="send"]')
        if (sendBtn) sendBtn.click()
      } else if (target.classList.contains('acctCodeInput')) {
        var verifyBtn = overlay.querySelector('#vlibAcctPanel [data-act="verify"]')
        if (verifyBtn) verifyBtn.click()
      }
    })
    document.addEventListener('keydown', onKey)

    // A dialog that opens without moving focus leaves a keyboard user typing
    // into whatever was focused behind it - the gear, or nothing at all.
    var closeBtn = overlay.querySelector('[aria-label="Close"]')
    if (closeBtn) closeBtn.focus()
  }

  // Everything Tab can reach, in DOM order. Recomputed on every keydown
  // rather than cached once, since render() rebuilds the tile rows (and
  // their buttons) under the dialog's feet as tiles are added or removed.
  function focusables() {
    if (!overlay) return []
  ...
  function close() {
    ...
    // Hand focus back to whatever opened this (the gear, almost always) so a
    // keyboard user lands exactly where they were, not at the top of the page.
  ...
  function onKey(e) {
    ...
    // Trap Tab inside the dialog. Without this, Tab walks off the modal and
    // into the greeting, the tile grid behind it - a sighted user sees the
    // scrim and assumes the page underneath is inert; a keyboard user finds
    // out it is not.
  ...
  // Sign-in redirect can land while the Library is already open (or reopened
  // right after) - keep the account panel honest without a manual refresh.
  //
  // An UNEXPECTED sign-out (nobody clicked Sign out - a refresh token that
  // expired, or iOS quietly clearing this home-screen app's storage) must
  // not just silently drop the board back to local-only. The board would
  // keep working, but it would be lying by omission: everything still looks
  // synced while nothing is going anywhere. The flag survives a full reload
  // (it is written to disk, not held in memory) so a sign-out that happens
  // while the board is closed is still reported the next time it is opened.
  var UNEXPECTED_SIGNOUT_KEY = 'v:unexpected-signout'
  function flagUnexpectedSignOut() {
    try { window.localStorage.setItem(UNEXPECTED_SIGNOUT_KEY, '1') } catch (e) {}
  }
  function clearUnexpectedSignOut() {
    try { window.localStorage.removeItem(UNEXPECTED_SIGNOUT_KEY) } catch (e) {}
  }
  function hadUnexpectedSignOut() {
    try { return !!window.localStorage.getItem(UNEXPECTED_SIGNOUT_KEY) } catch (e) { return false }
  }

  if (window.VitalityRemote) {
    window.VitalityRemote.onAuthChange(function (session, event, unexpected) {
      if (unexpected) flagUnexpectedSignOut()
      if (event === 'SIGNED_IN') clearUnexpectedSignOut() // back, and told so - resolved
      refreshAccount()
    })
  }
