...
;(function () {
  ...
  var MIGRATE_KEY = 'v:migrated'
  function alreadyMigrated() {
    try { return !!window.localStorage.getItem(MIGRATE_KEY) } catch (e) { return false }
  }
  function markMigrated() {
    try { window.localStorage.setItem(MIGRATE_KEY, '1') } catch (e) {}
  }
  function shouldOfferMigration() {
    return !alreadyMigrated() && window.VitalityRemote && window.VitalityRemote.hasLocalData()
  }
  ...
  function refreshAccount() {
    ...
    window.VitalityRemote.getSession().then(function (session) {
      if (session && session.user) {
        panel.innerHTML = shouldOfferMigration() ? migrateHTML(session.user.email) : signedInHTML(session.user.email)
      } else {
  ...
  function onAccountPanelClick(e) {
    ...
    if (act === 'send') {
      ...
      b.disabled = true
      window.VitalityRemote.migrateLocalData().then(function () {
        markMigrated()
        refreshAccount()
