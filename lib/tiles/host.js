...
;(function () {
  ...
  window.addEventListener('message', function (e) {
     ...
     * phone. If it cannot be scheduled - signed out, no vault, notifications
     * never turned on - the tile carries on counting down exactly as it always
     * has. That is also why an unknown message type is simply ignored: this
     * same tile file runs on the hosted Vitality board, which has no idea what
     * a restTimer is, and it must keep working there untouched.
     */
    if (msg.type === 'restTimer') {
      ...
      if (isFinite(secs) && secs > 0 &&
          ...
          window.VitalityPush) {
        // Only schedule if this device actually subscribed. Writing timer rows
        // for a device that can never receive them just fills the table.
        window.VitalityPush.isEnabled().then(function (on) {
          if (!on) return
          return window.VitalityRemote.scheduleRestPush(secs, msg.label || null)
